...
#define BINARY_FILE_PATH "STM32N6/Appli2/build/Appli-trusted.bin"
#define IRIS_PACKET_PAYLOAD_SIZE 1024U
#define INTER_PACKET_DELAY_US 1U
...
int main(void)
{
    ...
    printf("[Server] Streaming '%s' in %u packets\n", BINARY_FILE_PATH, total_packets);
    ...
    for (uint32_t packet_idx = 0; packet_idx < total_packets; ++packet_idx) {
        ...
        if ((packet_idx + 1U) < total_packets) {
